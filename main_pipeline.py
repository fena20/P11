...
def run_experiment(config, pipe_config, include_lights=True, perform_full_analysis=False):
    ...
    df_raw = pipeline_core.add_synthetic_negative_controls(df_raw, pipe_config)
    ...
    if perform_full_analysis:
        ...
        importance_df = compute_permutation_importance(
            ...
            random_state=42,
            method=str(config.get('settings', {}).get('perm_importance_method', getattr(pipe_config, 'perm_importance_method', 'standard'))),
            block_size=int(config.get('settings', {}).get('block_permutation_block_size', getattr(pipe_config, 'block_permutation_block_size', 144)))
        )
        importance_df.to_csv(outpath('feature_importance.csv'), index=False)
        
        logger.info("Auditing negative control variables...")
        baseline_mse = float(np.mean((y_test - y_pred_used) ** 2))
        audit_results = audit_negative_controls(
            importance_df,
            pipe_config.negative_control_cols,
            baseline_mse=baseline_mse,
            max_rel_mse_increase=float(config.get('settings', {}).get('negctrl_max_rel_mse_increase', 0.001)),
            z_threshold=float(config.get('settings', {}).get('negctrl_z_threshold', 2.0))
        )
...
if __name__ == "__main__":
    ...
    pipe_config = PipelineConfig(
        target_col=config['settings']['target_col'],
        forecast_horizon_h=args.forecast_horizon,
        include_lights=config['settings'].get('include_lights', True),
        enable_synthetic_negctrl=bool(config.get('settings', {}).get('synthetic_negative_controls', {}).get('enabled', True)),
        n_synthetic_negctrl=int(config.get('settings', {}).get('synthetic_negative_controls', {}).get('n_controls', 5)),
        synthetic_negctrl_prefix=str(config.get('settings', {}).get('synthetic_negative_controls', {}).get('prefix', 'negctrl_noise_')),
        synthetic_negctrl_distribution=str(config.get('settings', {}).get('synthetic_negative_controls', {}).get('distribution', 'normal')),
        synthetic_negctrl_ar1_phi=float(config.get('settings', {}).get('synthetic_negative_controls', {}).get('ar1_phi', 0.0)),
        perm_importance_method=str(config.get('settings', {}).get('perm_importance_method', 'block')),
        block_permutation_block_size=int(config.get('settings', {}).get('block_permutation_block_size', 144)),
        test_size_percent=config['settings']['test_size_percent'],
        n_outer_folds=config['settings'].get('n_outer_folds', 5),
        n_inner_folds=config['settings'].get('n_inner_folds', 3),
        use_full_hyperparameter_search=True,
        current_load_available=(not args.no_current_load) and bool(config.get('assumptions', {}).get('current_load_available', True))
    )
